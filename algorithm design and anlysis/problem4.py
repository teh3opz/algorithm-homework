...
算法分析：
数组元素互不相同，并且满足A[1]>A[2]、A[n-1]<A[n]，可以保证数组中至少存在一个局部最小值。用二分查找思想：取中间位置mid。
...
"""
第二问问题描述：
给定一个包含不同整数元素的数组A[1..n]，并且满足：A[1]>A[2]，且A[n-1]<A[n]。
如果某个元素比它左右两边的邻居元素都小，即A[x]<A[x-1]且A[x]<A[x+1]，那么称A[x]是一个局部最小值，要求找出数组中的所有局部最小值。
算法分析：
由于要找出所有局部最小值，必须检查每一个可能的位置。遍历数组中除首尾之外的所有元素，如果当前元素同时小于左邻居和右邻居，就将其加入结果列表。
因为每个元素只被访问一次，所以时间复杂度为 O(n)。
时间复杂度：O(n)
空间复杂度：O(k)，其中k为局部最小值个数
"""
def findAllLocalMin(A):
    result = []
...
#测试
A = [9, 3, 7, 2, 1, 4, 5]
result = findAllLocalMin(A)
print(f"所有局部最小元素为: {result}")
